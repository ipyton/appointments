    ...
    <div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-grow relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-50 focus:bg-white transition-colors"
              value={searchTerm}
          ...
          <div className="w-full md:w-64">
            <div className="relative">
              <select
                className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 focus:bg-white text-gray-700 appearance-none focus:ring-blue-500 focus:border-blue-500 transition-colors"
      ...
      {loading ? (
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center shadow-sm">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-14 w-14 mx-auto text-yellow-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <h3 className="text-xl font-medium text-yellow-800 mb-2">No events found</h3>
          <p className="text-yellow-700 text-lg">No events match your search criteria. Try adjusting your filters.</p>
          <button 
            onClick={() => { setSearchTerm(''); setSelectedCategory(''); }}
            className="mt-4 px-4 py-2 bg-yellow-200 text-yellow-800 rounded-lg hover:bg-yellow-300 transition-colors font-medium flex items-center mx-auto"
          >
            ...
            <Link href={`/user/events/${event.id}`} key={event.id} className="group">
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 h-full flex flex-col transform group-hover:-translate-y-1">
                ...
                <div className="p-5 flex-grow">
                  <h2 className="text-xl font-semibold mb-3 group-hover:text-blue-600 transition-colors">{event.title}</h2>
                  <p className="text-gray-600 mb-4">{event.description}</p>
                  
                  {/* Provider with avatar */}
                  <div className="flex items-center mb-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <div>
                      <div className="text-sm font-medium text-gray-900">{event.provider}</div>
                      <div className="text-xs text-gray-500">{event.publisher}</div>
                    </div>
                  </div>
                ...
                <div className="px-5 py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                  ...
                  <span className="text-sm text-white px-3 py-1.5 bg-blue-600 rounded-lg group-hover:bg-blue-700 transition-colors flex items-center">
                    Book Now
      ...
      <div className="mt-10 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200 shadow-sm">
